...
- Session 剧本默认结构：热身 → 爬升 → 高强度随机 → 缓释（各阶段比例以配置 `session_script` 实际值为准，总时长不得超过 `session_max_minutes`）。各阶段的强度常态区间与爬升节奏见 `references/playbook.md` 的「强度区间地图」与「爬升基准线」——全程在最低档附近徘徊 = 节奏塌了。
