...
- `min_output_intensity` 最小有效输出档（默认 30）：低档位无体感，非零目标自动抬到该档（0=关闭除外）；起步（从 0 开到最小档）允许直达、豁免步长/冷却限制；**安全路径豁免**——次安全词降级、急停归零不受此限
- 单次上调消耗上调预算（额度 = `max_step_up`）：上调按幅度消耗、下调按幅度回补（"降多少，允许再升多少"）；预算耗尽即进入冷却，距上次上调满 `step_up_cooldown_seconds`（默认 30s）预算自动回满；预算为 0 时的上调直接抛 `SafetyViolation` 拒绝（是否构成上调以钳制/抬升后的最终目标值判断）
...
- 持续时长推荐用相对值 `duration_rel`（0.0–1.0 = 单次上限的几成，如 0.5 = 上限一半）——LLM 直接猜绝对秒数普遍偏短（用户反馈"持续时间不及预期"）；`duration_seconds` 绝对秒兼容保留，两者同时给时 `duration_rel` 优先
...
- **节奏与拉扯（过程质量）**：禁止"问答→奖惩→下一拍"速通。每个 ask 至少 2-3 个来回（点评/追问/质疑后才裁决）；奖惩之后有回味台词；沉默用"在场感"施压；按 `session_max_minutes`（默认 30 分钟）均摊阶段时长，过半自查进度，最高张力段落落在 70%~85% 处。详见 `references/scenario-design.md` 节奏章节。
...
7. **命令后确认**：关键档位变更（特别是关机、降级）可在 device 命令带 `"verify":true` 或事后发 `{"cmd":"query_device"}`，daemon 主动查询设备真实状态回报 `device_state`（估值 vs 实际），不一致按实际值回写估值——增量估值会随设备衰减漂移，对账是确认"真的降下去了"的唯一可靠方式。
