...
1. 环境依赖验证（见「环境准备与依赖」）。
2. 连接 V4 Relay → 生成配对二维码 → **立即在对话中展示**（嵌入图片，见「设备接入」）→ 引导佩戴者用 DG-LAB 4 APP 扫码接入；失败时输出具体排查引导（Relay 连通性/设备开机/APP 版本），最多重试 3 次。
...
3. 客户端连接 Relay 得到控制方 ID，daemon 生成配对二维码并发出 `pairing` 事件（含 `qr_png` 图片路径）。**收到 `pairing` 事件必须立即在对话中嵌入二维码图片展示**——只生成不展示 = 流程失败，必须当场补救，不能让佩戴者干等。佩戴者用 DG-LAB 4 APP 扫码接入（APP 只能扫码，无法手输地址）。
